...
const RM_SYSTEM = "rmTopCMS";
const RM_VERSION = "1.0.0";
const RM_COPYRIGHT = "http://rmsf.top/";
const RM_EMAIL = 'hezuo@ynrmsf.com';
const RM_PHONE = '';
const RM_HOST = 'http://rmsf.top/';
const RM_INSTALL = 'http://install.rmsf.top';
const RM_UPGRADE = 'http://upgrade.rmsf.top';
const RM_AGREE = '
提示条款：
本协议是您与{$sys.Copyright}共同签署。
<strong>{$sys.Name}（以下称“{$sys.Name}”），由{$sys.Copyright}（以下称“{$sys.Name}”）独创开发，版权所有Copyright © {$sys.Name}，{$sys.Copyright}保留所有权利。{$sys.Name}是国内专业的cms解决方案之一，{$sys.Copyright}基于 PHP + MySQL 的技术，采用ThinkPHP框架开发。{:Name}官方对此拥有最终修改权和解释权。</strong>
{$sys.Name}官方网站：{$sys.Name}
<strong>在使用{$sys.Name}（以下称“许可软件”或“本软件”）之前，请您仔细阅读本协议，特别是法律适用和争议解决条款，此等条款将以粗体标识，您需要重点阅读。如您对协议有任何疑问，可向客服咨询。如果您已下载、复制、安装或以其他任何方式使用该软件，则视为已经接受本协议。如果您不接受本协议的全部或部分条款，您将无权使用本软件。请立即终止安装、或以其他方式使用该软件，删除您已经安装或保留的该软件的任何组件。</strong>
...
<br/>
一、定义
软件（许可软件或本软件）：本协议中的“软件”是指{$sys.Name}，由若干模块或功能组成的已经植入或即将植入{$sys.Copyright}指定产品内的信息处理程序或支持文件，其中支持文件具体包括软件的源代码、目标码以及相关软件中所包含的图片、照片、图标、动画、录音、录像、音乐、文字、代码的全部或部分，还包括与许可软件或众邦产品相关的所有描述其功能、特点、内容、质量、测试、用户手册、用户许可协议等纸质或电子版的资料、技术文档等。
您：本协议中的“您”是指取得{$sys.Copyright}合法许可使用本软件权利的个人或单个法人实体，法人实体包括公司、企业、机构、组织或单位。
我们：本协议中的“我们”即是{$sys.Copyright}官方，是指{$sys.Name}，即{$sys.Copyright}及其关联公司。
二次开发：本协议中的“二次开发”在现有的软件上进行定制修改，如对功能扩展，达到您想要的功能，原则不得改变本软件原有系统内核及系统设定的框架，我们允许的二次开发仅指对部分软件界面、功能删改或扩展，并非对内核及框架进行实质性修改。
<br/>
二、软件许可使用内容
在您遵守本协议内容的前提下，您通过我们指定合法渠道购买软件商用许可后，{$sys.Copyright}授予您的商用许可权利包括：
...
5、授权内容使用权：在取得我们许可后，您拥有使用本软件构建的网站全部内容使用权，并独立承担与之相关法律义务。您可以在协议规定的约束条件下和限制范围内修改 {$sys.Copyright} 源代码或界面风格以适应您的网站要求，但应保留我们的版权信息。不管你的网站是否整体使用 {:softAbbreviation} ，还是部份栏目使用 {:softAbbreviation}，在你使用了 {:softAbbreviation} 的网站主页上必须加上 {:softAbbreviation} 官方网址({:website})的链接。
6、在获得商业授权之后，您才可以将本软件应用于商业用途，同时依据所购买的授权类型确定技术支持内容。商业授权用户享有反馈意见和提出建议的权力，相关意见和建议将在我们下一次软件升级中被优先考虑，但我们对此不作承诺或保证。
7、{$sys.Copyright}著作权已在中华人民共和国国家版权局注册(中国国家版权局著作权登记号 99)，著作权受到法律和国际公约保护。未经我们书面许可，不得删除网站底部及相应的官方链接。购买商业授权请联系{$sys.Copyright}了解最新说明。
...
6、转让限制：未经{$sys.Copyright}的书面同意，您不得公开、转让、出租、出借、再许可、分发该软件的全部或任何部分或软件单一备份副本给第三方。
7、保密限制：未经{$sys.Copyright}书面同意，您不得将本软件的性能或其他任何评估、测试结果、技术秘密透露给任何第三方。
<br/>
四、权利保留
1、 {$sys.Copyright}依法保留未在本协议中明确授予给您的其他一切在法律上属于{$sys.Copyright}的权利。
2、本软件受著作权法、国际著作权条约和其他的知识产权法律或国际条约保护。根据本协议，在此仅许可您非独占性的、非排他性的一般许可使用该软件的权利，而不是出售或转让。
3、商标权：本协议不授予您{$sys.Copyright}或其供应商的任何商标或服务标志相关的任何权利。
4、本软件所涉及到的一切知识产权，包括但不限于专利权、著作权、商标权、商业秘密、技术秘密，均属于各自内容拥有者的财产，{$sys.Copyright}保留从其所拥有的知识产权获取利益的权利。
5、未经我们书面许可，不得对本软件或与之关联的商业授权进行出租、出售、抵押或发放子许可证。
6、未经我们书面许可，禁止在 {$sys.Copyright} 的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。
7、您一旦开始确认本协议并安装 {$sys.Copyright}，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权立即终止授权，责令停止损害，并保留追究相关责任的权力。
<br/>
五、知识产权
...
2、升级版本的许可：如果该软件经{$sys.Copyright}同意升级，除非升级版本有替代的软件许可协议，否则升级版本仍应遵循本协议条款。
3、不论软件是否升级，您必须遵守本协议。
<br/>
七、无担保和责任限制
1、除{$sys.Copyright}明确明示保证的事项以外，对其他任何默示、特定用途、适销性不做任何默示或明示的保证，由此引起的风险由您自己承担。
2、有关本软件在使用过程中存在不适用性情况，您应当立即以书面方式反馈给我们，在我们现有技术可以解决的情况下，将依照{$sys.Copyright}的软件产品标准保修政策规定。
1){$sys.Copyright}不对试用期及免费试用软件因使用而产生的损失承担任何明示或暗示的责任。
2){$sys.Copyright}承担的所有责任以您购买该软件所支付的价款为限。
3、对因意外事故、滥用、错误使用、擅自修改所引起的软件使用问题，我们不承担任何责任，也不做任何保证。对因软件产品存在被攻击，或者自然灾害等不可抗力因素或非{$sys.Copyright}原因导致软件不能使用，或造成损失的，我们不承担任何责任，也不做任何保证。
4、对因使用软件引起的其他任何附带的、间接的或惩罚性的损失，包括但不限于商业利润的损失、信息或数据的丢失，{$sys.Copyright}不承担任何责任，即使{$sys.Copyright}已被告知存在此种损害的可能性也不例外。
...
<br/>
九、协议终止和违约责任
1、如果您没有遵守本协议的部分或全部条款，{$sys.Copyright}可以随时单方终止本协议。协议终止后，我们将取消对您的商用许可授权，同时您必须立即停止使用该软件，对已经安装的软件进行卸载，如果由于您违反了本协议的规定给{$sys.Copyright}造成损失，应承担损失赔偿责任。
2、您应理解按授权范围使用许可软件、尊重软件及软件包含内容的知识产权、按规范使用软件、按本协议约定履行义务是您获取我们授权使用软件的前提，如您违反本协议，我们有权终止使用许可。
3、您对软件的使用有赖于我们及关联公司为您提供的配套服务，您违反与我们或我们关联公司的条款、协议、规则、通告等相关规定，我们有权终止使用许可。您违反了本协议的规定给{$sys.Copyright}造成损失，应承担给我们造成损失的赔偿责任。
...
<br/>
十、管辖法律及可分割性
1、<strong>本协议之效力、解释、变更、执行与争议解决均适用中华人民共和国法律，如无相关法律规定的，则应参照通用国际商业惯例和（或）行业惯例。本协议由您与我们于我们服务器所在地{$sys.Copyright}签署。因本协议产生或与本协议有关的争议，您可与我们以友好协商，协商不成时，提交西安仲裁委员会予以裁决。仲裁裁决是终局的，对双方均有拘束力。</strong>
